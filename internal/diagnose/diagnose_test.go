...
func TestGatherReportsVolumeAndCapability(t *testing.T) {
	...
	if runtime.GOOS != "linux" {
		if result.Capabilities[0].Available {
			t.Fatal("open-deleted capability unexpectedly available")
		}
		if result.OpenDeletedSummary != nil || len(result.OpenDeleted) != 0 {
			t.Fatalf("unsupported platform returned open-deleted data: %#v", result)
		}
	} else if result.OpenDeletedSummary == nil {
		t.Fatal("Linux diagnostics omitted open-deleted coverage summary")
	}
}
...
func TestGatherHonorsCancelledContext(t *testing.T) {
	...
	if runtime.GOOS == "linux" && (result.OpenDeletedSummary == nil ||
		!result.OpenDeletedSummary.Coverage.Canceled || result.OpenDeletedSummary.Coverage.Complete) {
		t.Fatalf("cancelled coverage = %#v", result.OpenDeletedSummary)
	}
}
