...
func bindFlags(root *cobra.Command, cfg *Config) {
	...
	pf.StringArrayVar(&cfg.IncludeFS, "include-fs", nil, "include ONLY these filesystem types (Linux; repeatable)")
	pf.StringArrayVar(&cfg.ExcludeFS, "exclude-fs", nil, "exclude these filesystem types (Linux; repeatable)")
